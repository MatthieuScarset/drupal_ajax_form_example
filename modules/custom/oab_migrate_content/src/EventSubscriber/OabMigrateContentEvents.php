  ...
  public function deleteUrlAlias(MigrateRowDeleteEvent $migrate_row){
            ...
            if ($correspondance['label_d8'] != "") {
                ...
                if (isset($entity) && !empty($entity) && count($entity) > 0) {
                    ...
                    \Drupal::logger('oab_migrate_content')
                        ->notice('Attention : pas de terme D8 correspondant à "' . $correspondance['label_d8'] . '" (' . $correspondance['langcode'] . ') ---- pour la taxonomie '.$vid_d_8.' - terme D7 = "' . $correspondance['label_d7'] . '" - TID D7 :' . $correspondance['tid_d7']);
                }
            }
            else {
                // on n'a pas de correspondance pour cette Taxonomie
                \Drupal::logger('oab_migrate_content')
                    ->notice('Attention : pas de correspondance D8 pour la taxonomie '.$vid_d_8.' - term = "' . $correspondance['label_d7'] . '" - TID D7 :' . $correspondance['tid_d7']);
            }
        }
    ...
    public function getAllOfficeCountries(MigrateImportEvent $migrate_row) {
        ...
        if ($migration->getPluginId() == "office_node") {
            ...
            $query = $db_d7->select('location_country', 'c')
                ->fields('c', ['code', 'name']);
            $countries_results = $query->execute()->fetchAll();

            //on remet la connexion D8
            \Drupal\Core\Database\Database::setActiveConnection();
            $db_d8 = \Drupal\Core\Database\Database::getConnection();
            ...
            if (is_array($countries_results)) {
                foreach ($countries_results AS $country) {
                    ...
                    $query = $db_d8->select('taxonomy_term_field_data', 't');
