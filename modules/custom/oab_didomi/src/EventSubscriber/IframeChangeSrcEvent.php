  ...
  public function onResponse(ResponseEvent $event) {
    ...
    foreach($xpath->query("//iframe[(contains(@src, 'youtube'))]") as $iframe) {
      ...
      if(count($block_didomi) > 0) {
        ...
        if(key_exists($this->languageManager->getCurrentLanguage()->getId(), $languages)){
          $block_didomi = $block_didomi->getTranslation($this->languageManager->getCurrentLanguage()->getId());
        }
